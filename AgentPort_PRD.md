...
| Idle | Turn 完成或输出静默；通常进程仍在（Claude、Codex、Kimi、Pi、Qoder 在官方 Hook/Adapter 确认主 Turn 完成后，持续 15 分钟无输入且无新的高置信度工作事件时自动停止；可安全验证已完成 Turn 的精确恢复会继承空闲计时。Shell 不依据 PTY 静默自动停止） | 空心圆；启发式时显示中置信度 | 新 Turn、退出或失联 |
