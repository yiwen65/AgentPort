...
impl AgentAdapter for ClaudeAdapter {
    ...
    fn build_launch(&self, ctx: &LaunchContext) -> Result<LaunchPlan> {
        ...
        let (assigned, resume_precision) = if super::has_flag(install, "session-id") {
            ...
            notices.push(LaunchNotice::new(
                "native_session_id_unavailable",
                "该版本无 --session-id，无法指定原生 Session ID，仅支持恢复最近的 Session",
            ));
        ...
        let resume_precision = match &ctx.agent_session_id {
            Some(id) => {
                if !super::has_flag(install, "resume") {
                    return Err(CoreError::Blocked(
                        "this version of claude has no --resume flag and cannot resume a Session exactly".into(),
                    ));
                }
            ...
            None => {
                if !super::has_flag(install, "continue") {
                    return Err(CoreError::Blocked(
                        "no native Session ID is available and this version of claude has no --continue flag".into(),
                    ));
                }
...
mod tests {
    ...
    fn checked_resume_falls_back_to_fresh_when_transcript_missing() {
        ...
        assert!(plan
            .notices
            .iter()
            .any(|notice| notice.code == "claude_conversation_missing"));
    }
