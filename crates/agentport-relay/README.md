...
pinned computer key and endpoint, asks for this phone's name, then creates a new
Noise identity inside its existing secure credential backend. A disabled profile
holding only the opaque credential handle/public metadata is saved before sending
any pairing handshake. Compare the transcript code on both screens and explicitly
approve on the computer. Only the native authenticated approval enables the profile.
