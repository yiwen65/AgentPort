...
- Prevention: Regression test stubs `devicePixelRatio=2` and asserts `elementFromPoint` receives the RAW position (200,100), not the halved one; ghost suppression asserts Linux swaps the image while macOS/unknown platforms keep the native one. When a platform quirk needs a predicate, keep it in `store.ts` (tests mock `./actions` wholesale).
