...
在“设置 → 备份与恢复”中，仅为已探测到本地安装的 Agent 显示独立的备份、合并恢复入口，并沿用 Agent 排序。未探测到安装项时，可到“Agent 适配器”页面重新探测；已有备份仍可查看和校验。

备份中会显示当前 Agent、执行阶段、阶段进度和已用时间。本页面一次执行一个备份或恢复任务，其余操作暂时禁用，成功或失败后恢复。数据库复制通过独立只读连接固定快照，不会因活跃 Session 持续写入而反复重来；快照步骤限时 120 秒，超时会明确报错。原生历史收集、打包和校验的耗时另计，可能随数据量增长。
