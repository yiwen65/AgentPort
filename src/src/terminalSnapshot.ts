 ...
 * Keep in lockstep with mobile/src/terminal/terminalSnapshot.ts and the
 * headless Host snapshot engine on xterm upgrades. */
...
/** The authoritative screen served by a live Host (`terminal_snapshot_v1`). */
...
export function isTerminalSnapshot(value: unknown): value is HostScreenSnapshot {
  if (!value || typeof value !== "object") return false;
  const s = value as HostScreenSnapshot;
